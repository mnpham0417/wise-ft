...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            torch.save(model_student, f"/scratch/mp5847/wise-ft-kd/{args.name}/model_{epoch}.pt")
            # model_student.save(os.path.join(f"/scratch/mp5847/kd-explore-official/{args.name}", f"checkpoint_{epoch}.pth"))

        if args.distributed:
            train_sampler.set_epoch(epoch)
        # train for one epoch
        train(train_loader, model_student, criterion_ce, criterion_kl, optimizer, epoch, args)
...
@record
def train(train_loader, student, criterion_ce, criterion_kl, optimizer, epoch, args):
    with torch.autograd.set_detect_anomaly(True):
        ...
        for idx, data in enumerate(train_loader):
            ...
            loss = criterion_ce(student_out, target)
