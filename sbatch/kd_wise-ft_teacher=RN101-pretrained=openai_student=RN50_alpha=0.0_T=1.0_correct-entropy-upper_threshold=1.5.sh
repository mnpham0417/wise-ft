...
#SBATCH --output=kd_wise-ft_teacher=RN101-pretrained=openai_student=RN50_alpha=0.0_T=1.0_correct-entropy-upper_threshold=1.5_%j.out
...
singularity exec --nv \
  ...
  /bin/bash -c 'source /ext3/env.sh; conda activate /scratch/mp5847/conda_environments/conda_pkgs/wise_ft; cd /home/mp5847/src/wise-ft-selected-entropy; export PYTHONPATH="$PYTHONPATH:$PWD"; \
      torchrun src/train_kd.py --name "teacher=RN101-pretrained=openai_student=RN50_alpha=0.0_T=1.0_correct-entropy-upper_threshold=1.5" \
                            --epochs 100 \
                            --learning_rate 0.1 \
                            --cuda \
                            --workers 5 \
