...
#SBATCH --output=kd_wise-ft_RN101-pretrained=openai_student=RN101_scaled_%j.out
...
singularity exec --nv \
  ...
  /bin/bash -c 'source /ext3/env.sh; conda activate /scratch/mp5847/src/comp_exp; cd /home/mp5847/src/wise-ft; export PYTHONPATH="$PYTHONPATH:$PWD"; \
      torchrun src/train_kd.py --name "teacher=RN101-openai-wiseft-alpha=0.000_student=RN101-openai_alpha=0.0_T=20.0_scaled" \
                            --epochs 100 \
                            --learning_rate 0.1 \
                            --cuda \
                            --workers 5 \
                            ...
                            --rank 0 \
                            --T 20.0 \
                            --teacher-logits-path /scratch/mp5847/wise-ft-precompute/RN101-openai-imagenet-logits_scaled.npy'
